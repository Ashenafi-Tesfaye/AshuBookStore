...
import com.ashenafiAG.spring.demo.AshuBookStorewithSpring.repository.AddressRepository;
...
public class AddressService {
	...
	public void getRecentlyUsedAddress(ModelMap model, Long userId) {
		...
		if(order == null) {
			...
			if(recentlyUsedAddress != null) {
				model.put("userAddress", formatAddress(recentlyUsedAddress));
			}
			else { 
					model.put("userAddress", null);
			}
		}else {
			Address recentlyAddedAddress = addressRepository.findById(order.getAddressId()).get();
			
			model.put("userAddress", formatAddress(recentlyAddedAddress));
